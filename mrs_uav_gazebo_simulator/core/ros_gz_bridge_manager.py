...
class RosGzBridgeManager():
    ...
    def generate_uav_ros_gz_config(self, robot_params: dict) -> tuple[str, dict]:
        ...
        if not self._has_attached_sensors(attached_sensors):
            self._ros_node.get_logger().info(
                f"There are no additional sensors. Skipping launching ros_gz_bridge for {uav_name}")
            return "", []

        sensor_topics = self._get_sensor_topics(attached_sensors)
    ...
    def _get_attached_camera(self, attached_sensors: dict, camera_sensor: Element) -> None:
        gz_camera_info_topic = self._get_sensor_topic_from_tag_name(camera_sensor, SdfTopicTags.GZ_CAMERA_INFO)
        ros_camera_info_topic = self._get_sensor_topic_from_tag_name(camera_sensor, SdfTopicTags.ROS_CAMERA_INFO)
        ros_color_image_topic = self._get_sensor_topic_from_tag_name(camera_sensor, SdfTopicTags.ROS_COLOR_IMAGE)
    ...
    def _get_attached_depth_camera(self, attached_sensors: dict, camera_sensor: Element) -> None:
        gz_camera_info_topic = self._get_sensor_topic_from_tag_name(camera_sensor, SdfTopicTags.GZ_CAMERA_INFO)
        gz_pointcloud_topic = self._get_sensor_topic_from_tag_name(camera_sensor, SdfTopicTags.GZ_POINTCLOUD)
        ros_camera_info_topic = self._get_sensor_topic_from_tag_name(camera_sensor, SdfTopicTags.ROS_CAMERA_INFO)
        ros_depth_image_topic = self._get_sensor_topic_from_tag_name(camera_sensor, SdfTopicTags.ROS_DEPTH_IMAGE)
        ros_pointcloud_topic = self._get_sensor_topic_from_tag_name(camera_sensor, SdfTopicTags.ROS_POINTCLOUD)
    ...
    def _get_attached_lidar(self, attached_sensors: dict, lidar_sensor: Element) -> None:
        ...
        gz_pointcloud_topic = self._get_sensor_topic_from_tag_name(lidar_sensor, SdfTopicTags.GZ_POINTCLOUD)
        ros_pointcloud_topic = self._get_sensor_topic_from_tag_name(lidar_sensor, SdfTopicTags.ROS_POINTCLOUD)
    ...
    # #{ _get_sensor_topic_from_tag_name(self, sensor, tag_name)
    def _get_sensor_topic_from_tag_name(self, sensor, tag_name):
    ...
    def _get_sensor_topics(self, attached_sensors: dict) -> dict:
        sensor_topics = {category: [] for category in RosGzBridgeCategory}
